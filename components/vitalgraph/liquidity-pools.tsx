...
export function LiquidityPools({ twin }: { twin: DigitalTwin | null }) {
  ...
  const [paymentMethod, setPaymentMethod] = useState<"SUI" | "USDC" | "x402">("SUI");
  const [x402Connected, setX402Connected] = useState(false);

  const handleCreatePool = () => {
    if (!newPoolName || !newPoolCriteria) return;
    setIsCreating(true);
    const tx = vitalService.createPool(newPoolName, newPoolCriteria);
  ...
  const handleDownload = async (recordName: string) => {
    toast.info(`Requesting data access for ${recordName}...`);
    setTimeout(() => {
      toast.success("Access Approved. Downloading encrypted blob...");
    }, 1500);
  };
            ...
            <AnimatePresence mode="popLayout">
                ...
                >
                    <Card
                        ...
                        pool.isNew ? "border-teal-500/50 bg-teal-500/5 shadow-[0_0_30px_-10px_rgba(45,212,191,0.3)]" : ""
                        }`}
                    >
                        ...
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <Badge
                                variant="outline"
                                className="mt-2 border-white/10 text-white/60 bg-black/20 font-normal"
                            >
                                {pool.criteria}
                            </Badge>
                            </div>
                            <div className="text-right">
                            <div className="text-xs text-white/40 uppercase tracking-wider mb-1">APY</div>
                        ...
                        <div className="flex justify-between text-sm text-white/40 mb-5 px-1">
                            <div className="flex items-center gap-2">
                                ...
                                Liquidity: <span className="text-white/70">{pool.liquidity}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-2 h-2 rounded-full bg-blue-500/50" />
                                Participants: <span className="text-white/70">{pool.participants}</span>
                            </div>
                        </div>
                        ...
                        {isResearcher ? (
                            ...
                            onOpenChange={(open) => !open && setExplorerOpen(false)}
                            >
                            <DialogTrigger asChild>
                                <Button
                            ...
                            <DialogContent className="bg-slate-950/95 backdrop-blur-xl border-white/10 text-white max-w-2xl shadow-2xl">
                                ...
                                <TabsList className="bg-black/40 w-full border border-white/5 p-1 h-auto">
                                    <TabsTrigger value="data" className="flex-1 py-2 data-[state=active]:bg-white/10 data-[state=active]:text-white text-white/60">Staked Data</TabsTrigger>
                                    <TabsTrigger value="fund" className="flex-1 py-2 data-[state=active]:bg-blue-500/20 data-[state=active]:text-blue-300 text-white/60">Fund Pool</TabsTrigger>
                                </TabsList>
                                ...
                                >
                                    ...
                                    <div className="flex flex-col items-center justify-center h-full text-white/40 gap-2">
                                        ...
                                        <span className="text-xs">Fetching on-chain events...</span>
                                    </div>
                                    ) : poolEvents.length > 0 ? (
                                    poolEvents.map((evt, i) => (
                                        ...
                                        <div>
                                            ...
                                            Provider: {evt.provider.slice(0, 8)}...
                                            </div>
                                        </div>
                                        <Button
                                            size="sm"
                                            variant="ghost"
                                            className="text-xs border border-white/10 hover:bg-teal-500/10 hover:text-teal-300 hover:border-teal-500/30 transition-all"
                                            onClick={() =>
                                            handleDownload(evt.recordName)
                                            }
                                        >
                                    ...
                                    <div className="flex flex-col items-center justify-center h-full text-white/40 border border-dashed border-white/10 rounded-xl">
                                ...
                                <TabsContent value="fund" className="mt-4">
                                    ...
                                    <div className="grid grid-cols-3 gap-3 mb-6">
                                        <button 
                                            ...
                                            className={`flex flex-col items-center justify-center p-3 rounded-lg border transition-all ${paymentMethod === "SUI" ? "bg-blue-500/20 border-blue-500 text-white" : "bg-black/20 border-white/10 text-white/40 hover:bg-white/5"}`}
                                        >
                                        ...
                                        <button 
                                            ...
                                            className={`flex flex-col items-center justify-center p-3 rounded-lg border transition-all ${paymentMethod === "USDC" ? "bg-blue-500/20 border-blue-500 text-white" : "bg-black/20 border-white/10 text-white/40 hover:bg-white/5"}`}
                                        >
                                        ...
                                        <button 
                                            ...
                                            className={`flex flex-col items-center justify-center p-3 rounded-lg border transition-all ${paymentMethod === "x402" ? "bg-purple-500/20 border-purple-500 text-white" : "bg-black/20 border-white/10 text-white/40 hover:bg-white/5"}`}
                                        >
                                        ...
                                        {paymentMethod === "x402" ? (
                                            <div className="text-center py-4">
                                                <div className="mb-4 text-sm text-purple-200/70">
                                                    Stream payments instantly via x402 Protocol.
                                                    <br/>Pay-per-second for real-time data access.
                                                </div>
                                                <Button 
                                                    onClick={handleFundPool}
                                                    className={`w-full h-12 ${x402Connected ? 'bg-red-500/20 text-red-300 border-red-500/50' : 'bg-purple-600 hover:bg-purple-500 text-white'}`}
                                            ...
                                            <>
                                                <div className="space-y-2">
                                                    <Label className="text-blue-200/60 text-xs uppercase">Amount ({paymentMethod})</Label>
                                                    <Input
